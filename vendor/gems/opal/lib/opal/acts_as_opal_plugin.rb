...
module Opal
  module ActsAsOpalPlugin
    module ClassMethods
      ...
      # get plugin record for this class
      def plugin
        result = Plugin.where("name = ?", system_name).first
        return result
      end

      # the plain, system name of the plugin, ie: Image
      def system_name
        result = name.gsub("Plugin", "")
        return result
      end
      ...
      def can?(performer, action, options = {})
        ...
        when User
          case action.to_sym
          when :create, :new
            GroupPluginPermission.for_plugin_and_group(
              plugin, performer.group).can?(action) ||
              (options[:record] ? options[:record].is_user_owner?(performer) : false) ||
    ...
    module InstanceMethods
      ...
      def set_approval
        if respond_to?(:is_approved) && self.user
          # approve if not required or owner or admin
          if !GroupPluginPermission.for_plugin_and_group(
            self.class.plugin, self.user.group).
            requires_approval? ||
            record.is_user_owner?(user) ||
            user.is_admin?
            self.is_approved = "1"
          end
        end
...
module ActiveRecord
  class Base
    # for use in plugins
    def self.acts_as_opal_plugin(options = {})
    # Option Defaults
