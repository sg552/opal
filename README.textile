...
thanks to Hulihan, this is a fork from his "opal" project. 
(see: https://github.com/hulihanapplications/Opal)

h2. TODOs:
OPAL 搭建好了。需要:
1.  i18n 中文化～～～  ^_^
2.  layout
3.  import data & setup demo site
4.  所有的单元测试都通过

1.  在OPAL基础上做开发
2.  使用 aws 服务器
3.  先用MYSQL， 以后再转到MONGODB。

其他：
4.  RSV 放上去
5.  做个个人主页，放上自己的介绍。
6.  完善其他项目，例如dudufish (forum) 等等。
7.  ZWSJ论坛。


h1. 安装流程：

1. database.yml  ，根据它建立数据库 
2. 确保config目录中的文件都存在，不存在的 cp ooxx.sample ooxx
3. 手动建立database之后， rake db:migrate  等等常规操作 。
