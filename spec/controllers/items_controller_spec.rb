  ...
  describe "as admin" do
    ...
    describe "set_preview" do
      it "should work with the right params" do
        item = Factory(:item)
        item_id = item.id
        post :set_preview, {:id => item,
          :preview_id => Factory(:plugin_image, :record => item).id,
          :preview_type => PluginImage.name}
        flash[:success].should_not be_nil
        Item.find(item_id).preview_type.should == PluginImage.name
