class Setting < ActiveRecord::Base
  ...
  def self.get_global_settings
    begin
      ...
      # convert array to hash, indexed by name, value by to_value
  	  setting = setting_array.hash_by(:name, :to_value)
      Rails.logger.info "=== now in get_global_settings,  setting: #{setting.inspect}"
  	  setting[:title] = setting[:site_title]
  	  setting[:description] = setting[:site_description]
      # url for theme directory
  	  setting[:theme_url] =  "/themes/#{setting[:theme]}"
  	  setting[:themes_dir] =  File.join(Rails.root.to_s, "public", "themes") # system path for main themes directory
      # system path for current theme directory
  	  setting[:theme_dir] =  File.join( setting[:themes_dir], setting[:theme])
