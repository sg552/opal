class Uploader #< ActiveRecord::Base
  ...
  def self.generate_image(options = {})
     ...
     # resized image width
     options[:image_width]        ||= default_value( @plugin.get_setting("item_image_width"), 500)
      # resized image height
     options[:image_height]       ||= default_value(@plugin.get_setting("item_image_height"),500)
      # thumbnail width
     options[:thumbnail_width]    ||= default_value(@plugin.get_setting("item_thumbnail_width"), 100)
      # thumbnail height
     options[:thumbnail_height]   ||= default_value(@plugin.get_setting("item_thumbnail_height"), 100)
