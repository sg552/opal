  ...
  def category_select_tag(name, value = nil, options = {})
    ...
    html += content_tag(:div,
      content_tag(:table,
        ...
        ;
        content_tag(:td,
          options[:include_blank].is_a?(String) ?
            options[:include_blank] :
            I18n.t("single.none"), :align => "left") +
        content_tag(:td, radio_button_tag(name, "", (
          value.blank? || value.to_s == "0")),
          :align => "right") ;
        end,
        :style => "width:100%",
        :cellpadding => "0", :cellspacing => "0"),
      :class => "indent", :include_blank=> "true") + tag(:hr) if options[:include_blank]
